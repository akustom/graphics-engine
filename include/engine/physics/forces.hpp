...
namespace engine::phy {
    inline void gravity(scene::Instances& instances) {
        ...
        simd::__m256 m256_g_constant = _mm256_set1_ps(G);
        ...
        for (std::size_t i = 0; i < items; i++) {
            ...
            simd::__m256 m256_self_pos  = simd::_mm256_set1_vec4(self_pos);
            simd::__m256 m256_self_mass = simd::_mm256_set1_vec4(self_mass);

            for (std::size_t j = 0; j < tail; j += simd::OFFSET_256::vec4) {
                simd::__m256 m256_other_pos   = simd::_mm256_loadu_vec4(&instances.instancesPos()[j]);
                simd::__m256 m256_other_force = simd::_mm256_loadu_vec4(&instances.instancesForce()[j]);
                simd::__m256 m256_other_mass = simd::_mm256_alignf_vec4(
                    instances.instancesMasses()[j],
                    instances.instancesMasses()[j+1]);

                simd::__m256 m256_diff = simd::_mm256_sub_vec4(m256_self_pos, m256_other_pos); // self_pos - other_pos
                simd::__m256 m256_dir  = simd::_mm256_norm_vec4(m256_diff); // norm(self_pos - other_pos)

                simd::__m256 m256_denom = simd::_mm256_length2_vec4(m256_diff); // mag(self_pos - other_pos)^2

                simd::__m256 m256_numer = simd::_mm256_mul_vec4(m256_dir, m256_other_mass); // dir * m1
                ...
                simd::__m256 m256_force = simd::_mm256_div_vec4(m256_numer, m256_denom); // dir * m1 * m2 * G / mag^2

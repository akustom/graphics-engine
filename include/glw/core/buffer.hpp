...
namespace glw {
    struct Buffer {
        ...
        void allocateBuffer(std::vector<T>& init_data, const GLenum flag = 0) const {
            glNamedBufferStorage(id, util::sizeof_vec(init_data), init_data.data(), flag);
        }

        template <trivially_copyable T>
        void allocateBuffer(uint32_t size, const GLenum flag = 0) const {
            glNamedBufferStorage(id, sizeof(T) * size, nullptr, flag);
        }

        template <trivially_copyable T>
        void pushData(const std::size_t byte_offset, std::vector<T>& data) const {
            glNamedBufferSubData(id, byte_offset, util::sizeof_vec(data), data.data());
        }
        template <trivially_copyable T>
